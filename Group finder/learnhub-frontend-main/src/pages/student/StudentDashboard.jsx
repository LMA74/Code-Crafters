  ...
  return (
    <>
      ...
      <div className="row g-3 mb-3">
        <div className="col-md-4"><div className="stat-card"><div className="text-muted">Enrolled study groups</div><div className="h2">{courses.length}</div></div></div>
      ...
      <div className="content-panel">
        <div className="d-flex justify-content-between align-items-center">
          ...
          <Link className="btn btn-primary btn-sm" to="/study-groups">Browse more</Link>
        </div>
        <hr />
        {courses.slice(0, 5).map((course) => (
          <div className="d-flex justify-content-between align-items-center border-bottom py-2" key={course.enrollment_id}>
            <span>{course.title}</span>
            <Link className="btn btn-outline-primary btn-sm" to={`/student/study-groups/${course.id}/learn`}>Open</Link>
          </div>
        ))}
      </div>
    </>

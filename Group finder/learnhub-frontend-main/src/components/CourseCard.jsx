  ...
  return (
    <div className="course-card d-flex flex-column">
      ...
      <div className="small text-muted mb-3">
        <div>Group leader: {course.teacher_name || 'You'}</div>
        <div>Duration: {course.duration || 'Self-paced'}</div>
      </div>
      <div className="d-flex flex-wrap gap-2">
        <Link className="btn btn-outline-primary btn-sm" to={`/study-groups/${course.id}`}>Details</Link>
        {onEnroll && <button className="btn btn-success btn-sm" onClick={() => onEnroll(course.id)}>Enroll</button>}
        {showManageLink && <Link className="btn btn-primary btn-sm" to={`/group-leader/study-groups/${course.id}/lessons`}>Manage</Link>}
      </div>
    </div>
  );
}
